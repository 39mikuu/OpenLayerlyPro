...
- **生成器、API 校验、登录 UI、文案和测试必须一起改**:
  ...
  - `src/components/auth/login-form.tsx` 当前 `maxLength={6}`、numeric-only `inputMode`/placeholder 必须按最终字符集与长度同步，不能让浏览器 UI 无法输入有效码;
  - i18n 错误/提示文案及前后端测试一并更新。生成集合、API 接受集合、UI 可输入集合三者必须完全一致。
...
- **生成↔API↔UI 同源**:`generateLoginCode` 产出的码能通过 verify-code 请求 schema，也能在登录表单完整输入/提交;新字母表/长度不受旧 `^\d{6}$`、`maxLength={6}` 或 numeric-only input hints 阻断;畸形码仍 400;i18n 文案更新。
...
- [ ] **生成↔API↔UI 同源**:`generateLoginCode` 字母表/长度 + verify-code schema + `login-form.tsx` 的 maxLength/inputMode/placeholder + i18n + 前后端测试一起改;新码不被旧 `^\d{6}$` 或 6 字符 UI 阻断
...
- [ ] 真实测试:**#66 跨多 IP 仍不能锁死受害者正确码**;email 变体不能绕过;Turnstile 不回退;request-code 异 IP 不被纯 email 锁;抑制不重复发信且不铸造未发送新码;unresolved 不波及 resolved;回归绿
...
> **修订说明(回应评审)**:#66 改为**核心语义修复**(正确码优先、不可被失败计数作废),限流退为纵深;**因移除全局上限,新增非锁死暴力兜底 = 提高验证码熵(§3.3)**;验证码生成/API/UI 三端同源;所有 email 维度状态先规范化再 hash/去重;request-code **保留 Turnstile**并删除纯 email 阻断,防轰炸用非阻断发信抑制(抑制不刷新时间戳、也不插入未发送的新码)+ 如实记录分布式投递残余;emergency 桶**如实表述**为「每操作共享、与 resolved 分离、unresolved 间不隔离」,且 **unresolved 只走一层、不双计**。