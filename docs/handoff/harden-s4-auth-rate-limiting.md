...
- [ ] 真实测试:**#66 跨多 IP 仍不能锁死受害者正确码**;request-code 异 IP 不被纯 email 锁;抑制不重复发信且不铸造未发送新码;unresolved 不波及 resolved;回归绿
...
> **修订说明(回应评审)**:#66 改为**核心语义修复**(正确码优先、不可被失败计数作废),限流退为纵深;**因移除全局上限,新增非锁死暴力兜底 = 提高验证码熵(§3.3)**;验证码生成/API/UI 三端同源;request-code **删除纯 email 阻断**,防轰炸用非阻断发信抑制(抑制不刷新时间戳、也不插入未发送的新码)+ 如实记录分布式投递残余;emergency 桶**如实表述**为「每操作共享、与 resolved 分离、unresolved 间不隔离」,且 **unresolved 只走一层、不双计**。