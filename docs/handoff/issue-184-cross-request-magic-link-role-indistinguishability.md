...
- **状态**：Proposed（v24，按三轮独立复核 F1–F12、F184-01–F184-15、F-A–F-AM 及 Codex 多轮复核修订；规则以 R-编号索引，见 §5 引用约定。v24 明确 F-AG 清单为 fresh terminal 与任意年龄非空围栏的并集，并用第 201 行精确判定监控截断）
...
F-AG 的维护模块只复用 task/protocol 关联与排序，**不得复用 stuck scan 的 non-NULL-only 谓词**。其 keyset `list` 的权威集合为 terminal protocol-v2 pending candidate 且满足 `(task.updated_at >= retentionCutoff OR delivery_reservation_until IS NOT NULL)`：即尚未进入正常 cleanup 的 fresh terminal（reservation 可为 NULL 或非 NULL）与任意 age 的非空围栏之并集。按 `(created_at,id)` 每页查询 201、返回最多 200 与精确 `nextCursor`，直到 cursor 为空才算完整枚举；回滚 gate 要求逐页列尽所有 stuck 项并逐项 retry/disposition，不能把监控样本当全量清单。周期扫描无需持久游标，因为它的职责是持续证明“至少存在 stuck fence”并给出有界样本，而不是自动处置；完整性由显式 keyset list 提供。这样即使进程在 terminal commit 与 hook 之间崩溃，下一 tick 也会重新发现并告警，同时不会牺牲围栏。
...
4. **处置 terminal v2 delivery candidate（F-AG / F-AK，回滚专用，逐项留证）**：常规 reconciler 只删除 `succeeded` / `dead`、已超过 retention、且 reservation 为 NULL 的 candidate。回滚查询必须通过上述并集谓词的 `(created_at,id)` keyset `list` 覆盖两组：未到 cutoff 的 fresh terminal pending（包括 reservation NULL），以及**任意 age 但 reservation 非空**的 terminal pending；必须翻页到 `nextCursor=null`，后一组永远不能被常规 cleanup 删除。两组都逐项选择：
...
26r. **`安全删除与 stuck 扫描互不饥饿`（F-T / F-AK / F-AM）**：>200 active、>200 live/retryable/fresh-terminal、>200 aged terminal + 非空 reservation 都不得阻塞更新的 `dead + pending + NULL + 已过 retention` 在单轮被删。独立 stuck 扫描查询 201、对外最多 200；分别以恰好 200 与 201 个匹配项断言 `truncated:false/true`，零删除/清 reservation。F-AG `list` 的 fixture 同时含 fresh NULL、fresh non-NULL、aged non-NULL 与 aged NULL：前三类用 keyset 翻页后恰好枚举且无重复/遗漏，aged NULL 在正常 cleanup 后不属于清单。legacy 不选；移除独立扫描、错误复用删除 driver 或无 cursor 的全量假象必须 RED。
...
| tasks dispatcher/index 对应测试文件 | queue 隔离；NULL 安全删除；non-NULL stuck 扫描 201-lookahead/200 样本；hook 崩溃恢复；F-AG 并集 keyset 完整枚举 |
...
| `src/modules/auth/magic-link-rollback.ts` + integration test | F-AG 并集谓词的 `(created_at,id)` keyset list（201 lookahead/200 返回）与处置；覆盖 fresh NULL + 任意 age 非空、两模式与审计 |
...
- 复核清单必须逐条确认：
  ...
  23. 见 §5.5a R-FT/F-AK/F-AM：安全删除只处理 aged terminal + reservation NULL；独立 bounded 扫描在 hook 前崩溃后仍发现任何 age 的 terminal + non-NULL、持续聚合告警，F-AG 以 fresh（含 NULL）∪ any-age non-NULL 的并集 keyset list 分页完整枚举，不修改围栏。
