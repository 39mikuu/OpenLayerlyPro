...
- **状态**：Proposed（v14，按三轮独立复核 F1–F12、F184-01–F184-15、F-A–F-AE 及 Codex 十一轮复核修订）
...
本文件是 Issue #184 的自包含权威规格。**v1 设计（三层内存预算）已被完全废弃**：独立复核 F1 指出它在「admin = 只读事务 / member = 写事务」上无法满足 Issue #184 不变量 1 的 latency-class 要求，而该不变量**不接受削弱、重新解释、豁免或修订**。v2 改为「角色无关的持久 intake + 通用处理任务」；v3–v5 修复重试时限、滚动发布安全、告警缺口、清理事务位置与队列吞吐等发现；v6 修复「mint 即作废已投递链接、但 SMTP 尚未成功」的可用性缺陷：替代 token 先以 pending/inactive 状态持久化，SMTP 成功后才在围栏事务中激活，并同时 supersede 旧的已投递 token；v7 固定跨 pending/active 的单调激活围栏、管理员角色判定的权威时点，以及 task 终态提交后的 pending-candidate 可重试清理；v8 补齐三处只靠事后检查而缺少串行化的边界——用可线性化的投递预留把管理员晋升与在途 SMTP 互斥（§5.3b）、让激活事务持 task 行锁并锁定该邮箱全部 token 行（消除租约抢占与「消费后仍激活」造成的第二个 session）、以及为 intake 行与 intake 门/cap 组合补上行锁与 fail-closed 校验；v9 撤回「三个 SMTP 超时之和 = 调用墙钟上界」这一错误推导（`socketTimeout` 是空闲超时），改用受围栏的预留续期 + 可中止硬期限，并修复终态 candidate reconciler 会被大量成功投递饿死、以及 intake task 行无保留期而无界增长两处问题；v10 要求围栏丢失时**立即**中止 SMTP 而不是等到硬期限、在确认拆除连接后清除自己写的预留以使晋升等待上界真正成立、把「租约过期但无人接手」的陈旧 claim 从成功 no-op 改为可重试失败（`markTaskSucceeded()` 不校验 `lease_until`，否则静默丢失），并给 reconciler 驱动查询补上终态引用过滤以消除第二种饥饿；v11 消除两处自相矛盾（失败后是否清预留、陈旧 claim 的终态），把激活事务的锁集合从「该邮箱全部行」收窄为可证明有界的最小集并撤回「已有 token 清理作业」这一不实前提，且新增回滚门：滚回旧镜像前必须显式过期全部非 active v2 token，否则被取消/被 supersede 的链接会复活并制造第二个 session；v12 把 F-X/F-Y/F-Z/F-AC 四条规范同步到此前遗漏的流程块、失败语义表、竞态表与复核清单，并新增 F-AE 区分「已证实围栏丢失」与「结果不确定」的任务终态；v13 不改任何设计决定，只做结构性收敛：新增 §4.3 规范性优先级、固定结果词汇与条款登记表，并把 §5.11、§7、§13 从**复述**规范内容改为只给「情形 → 结果词汇 → 权威条款」的索引，消除「一条规则散落四五处、改一处漏其余处」这一反复产生矛盾的根源;v14 修正 F-AE 的一个不成立推断——续期谓词自含 `lease_until`，故「影响 0 行」可能只是本 worker 租约过期而无人接手，必须复读任务与 candidate 才能分类；同时把 F-AC 的最小锁集合落到 §5.3a 流程块、verify/consume 与续期段落，并把晋升围栏的锁集合同样收窄为 pending 行。
...
### 4.3 规范性优先级、结果词汇与条款登记表

前十轮复核里，**反复出现的缺陷类型不是设计错误，而是同一条规则被写在多处、改一处漏其余处**。§5.11 失败语义表、§7 竞态表、§13 复核清单原本都在**复述** §5 的规范内容，于是每新增一条规则就有四五个副本要同步，漏掉的那个必然被下一轮评审当成矛盾报出来。本节消除该结构。

**优先级（规范性）**

1. §5 各节中带 **F-编号** 的段落是每条规则的**唯一权威处**。
2. §5.11、§7、§13 以及本文件其它表格是**索引**，不是规则来源：它们只给出「情形 → 结果词汇 → 权威条款编号」，不复述条款内容。
3. 若索引与权威条款出现任何不一致，**以权威条款为准**，并把该索引行视为待修缺陷（而不是设计冲突）。
4. §10 的测试是断言，允许携带具体判据；每条测试必须标注它固定的条款编号。
5. §5 内的 ASCII 流程块是**步骤顺序**的权威，但**不得复述某条 F-条款拥有的谓词**——只能写「按 F-xx」。第十一轮的 F-AC 残留正是流程块复述谓词造成的：v13 只去掉了索引表这一层复述，没有去掉 §5 内部这一层。

**结果词汇（固定集合，索引表只能用这些词）**

| 词汇 | 含义 |
|---|---|
| `成功 no-op` | 任务提交为 `succeeded`，不产生任何业务副作用 |
| `terminal no-op` | 已**证实**所有权易主或 candidate 已非 pending 后的成功收敛；重试归新持有者 |
| `可重试失败` | 抛瞬时错误，任务回到 `failed` 并按 `taskBackoffMs` 重试 |
| `永久失败` | `PermanentTaskError` / 首次尝试即 dead-letter |
| `整体回滚` | 事务全部回滚，持久状态不变 |
| `fail closed 回滚` | 运营操作被拒绝并回滚，抛可重试 `ApiError` |
| `不激活` | candidate 不转 `active`，不 supersede，不记 `magic_link_sent` |
| `旧链接保持原状` | 既有 active token 的有效期与可消费性不受影响 |
| `中止发送` | 立即拆除底层 socket，停止续期 |
| `告警` | 发出 §5.10 指定的可观测告警 |
| `只记日志` | best-effort，不回滚、不使任务失败 |
| `串行化` | 由所述锁排序，不并发交错 |
| `不可能` | 由所述锁结构排除，不需要额外补偿 |
...
| F-E | mint 上限回落是值推导，属 `rate-limit-policy.ts`，不得放进 `assertRuntimeSecurity()` | §5.8 |
| F-J | 备份恢复会产生一次与在途 intake 等量的超龄告警突发，属预期假阳性 | §7 恢复行 |
| F-K | `locale` 校验失败必须整键省略，不得写显式默认值 | §5.3 |
| F-M | `MAGIC_LINK_INTAKE_ENABLED` 只有精确字符串 `"true"` 为真 | §5.8 |
| F-N | intake 行必须 `FOR UPDATE` 锁定并持有到提交 | §5.3 |
| F-O | task 行必须 `FOR UPDATE` 锁定并持有到提交（事务 A 与 B 同样适用） | §5.3 |
| F-P | 只锁未终态行不足以与并发消费串行化 | §5.3a |
| F-Q | 晋升与在途 SMTP 由持久投递预留可线性化互斥 | §5.3b |
| F-R | 发布门开启时 intake cap `< 1` 必须 fail closed | §5.8 |
| F-S | 三个 SMTP 超时之和**不是**墙钟上界；覆盖由受围栏的预留续期提供 | §5.3b |
| F-T | reconciler 必须由 pending candidate 驱动，不得由 terminal task 驱动 | §5.5a |
...
| F-X | 围栏丢失（含续期抛错）必须立即中止发送，不得等硬期限 | §5.3b |
| F-Y | socket 确认拆除且围栏在己方手中时，必须按精确值清除自己写的预留 | §5.3b |
| F-Z | 陈旧 claim 分流：他人接手 → `成功 no-op`；租约过期且无人接手 → `可重试失败` | §5.3 |
| F-AA | `magicLinkDeliveryInFlight` 必须补齐 zh/en/ja 三个目录 | §2.4 |
| F-AB | reconciler 驱动查询必须过滤为终态且过保留期的引用 | §5.5a |
| F-AC | 激活事务的 token 锁集合是有界最小集，不是该邮箱全部行；基线无 token 清理 | §5.3a |
| F-AD | 滚回旧镜像前必须显式过期全部非 active v2 token | §9.3 |
| F-AE | 续期失败后必须复读任务与 candidate 再分类；只有已证实易主/已终结才是 `terminal no-op`，租约过期而无人接手、抛错、复读失败一律 `可重试失败` | §5.3b |

新增或修改规范时的要求：**只改权威处**，然后确认索引行的「结果词汇 + 条款编号」仍然正确。索引行不需要跟着改写措辞——这正是本节要消除的工作量。
...
**授权与围栏**：worker 不接受任何调用方传入的角色或身份；角色只在第 7 步由数据库读出。claim 校验 + 等锁后复检的双重围栏与 `deliverMagicLinkEmailTask()` 完全一致；陈旧执行一律**不得 mint**，但其终态**必须按 F-Z 分流**：被他人重新 claim 时是成功 no-op，租约过期而无人接手时必须抛可重试错误。此处不得笼统写成「一律成功 no-op」——那正是 F-Z 指出的静默丢失。
...
**事务 B 为什么不能只锁未终态行（F-P，规范性）**：`consumed_at` 是**终态**，因此「只锁未终态行」的旧写法既看不到、也不会与并发消费串行。于是下列交错成立：旧 active token 的消费事务先提交并插入 session，事务 B 随后照常激活 replacement，用户手里就同时有一个已用 session 与一个仍可消费的新链接——第二次消费会插入**第二个** session。这直接推翻 §7 的「最多一个 session」与测试 26i 的 `total session ≤ 1`。消费路径不取 email advisory lock（§7），所以唯一可用的串行化手段就是共同的 token 行锁。

**但锁「该邮箱全部行」是错的（F-AC）**：v9 写成无状态过滤地锁定全部行，并声称规模「由 §5.7 的 token 清理与保留期约束」。**该约束不存在**——我核实过，基线仓库中没有任何 `magic_link_tokens` 的删除或保留期作业（除 schema 定义与测试 truncate 列表外无引用），§5.7 只删 `magic_link_requests` 与 succeeded intake task，§5.5a 的 reconciler 只删终态 task 引用的 pending candidate。因此「全部行」会随邮箱历史无限增长，每次激活都要锁住越来越多的 consumed / expired / superseded / cancelled 行，拖慢激活与并发消费。
...
**verify / consume 查询**：所有按 hash/id 取得可验证 token 的查询必须追加 `delivery_state = 'active' AND delivered_at IS NOT NULL`；`pending`、`cancelled`、`superseded` 永远返回与无效 token 相同的公开结果。消费事务继续锁 token 后锁 user；由于事务 B 按 F-AC 锁定的最小集**必然包含任何可被消费的行**（未消费、未过期的 active/pending），它与激活事务竞争时只有两种串行结果：(a) 旧 token 先消费成功并提交 → 事务 B 读到 `consumed_at >= candidate.created_at`，把 candidate 置 `cancelled` 而**不激活**，用户手里不会多出一条仍可消费的链接；(b) 事务 B 先提交 → 旧 token 已 `superseded`，其消费返回 invalid。
...
**预留（写入方）**：事务 A 在同一事务内完成「锁定读到 non-admin」与「写 `candidate.delivery_reservation_until = now() + MAGIC_LINK_DELIVERY_RESERVATION_SECONDS`」，两者原子。事务 B 激活或取消时清空该列。SMTP 失败时的处理由 F-Y 统一规定：**socket 已确认拆除**就清除自己写的预留，否则（围栏已丢失，或无法确认连接已断）才任其自然到期。v9 曾一律「失败不清空」，那与 F-Y 直接冲突，且正是 §5.3b 残余 2 里算错上界的原因，此处撤回。
...
2. SELECT magic_link_tokens WHERE email = $1 AND delivery_state = 'pending' FOR UPDATE
      **只锁 pending 行**：本流程只关心「有无未到期预留」与「取消 pending candidate」，
      两者都只涉及 pending；锁全部行会随邮箱历史无限增长（同 F-AC 的理由）。
...
**时限：为什么不能用「三个超时之和」作为墙钟上界（F-S，规范性）**
...
**围栏丢失必须立即中止发送，不能等硬期限（F-X，规范性）**：v9 只让围栏丢失置一个标志、任由 SMTP 跑到硬期限，这不够。围栏丢失意味着**没有人再延长预留**；最后一次确认的到期时刻一过，晋升事务就能提交，而旧 socket 仍在发送，SMTP 服务器随后接受该消息——排序不变量被破坏。事务 B 失败关闭只能阻止激活，**无法撤销已经发出的邮件**。既然 §5.3b 已经要求传输层可中止，就必须在围栏丢失的那一刻用掉这个能力。「续期抛错」同样按丢失处理：无法确认预留被延长时，覆盖是不确定的，不确定必须按最坏情况处理。

**终态后必须清除预留（F-Y，规范性）**：只要 **socket 已确认拆除**（硬期限中止、SMTP 抛错、或正常返回后事务 B 已处理完），本次尝试就不可能再发出任何邮件，因此清除预留是安全的，且是让 §11.11 的等待上界真正成立的前提（见下）。清除必须同时受任务 claim 与**精确值**保护：
...
**「已证实丢失」与「结果不确定」必须给出不同的任务终态（F-AE，规范性）**：F-X 把「续期影响 0 行」与「续期抛错」都归为围栏丢失并要求立即中止发送，这对**外部副作用**是对的，但两者的**任务终态**不能相同。
...
| 复读结果 | 已知事实 | 任务终态 |
|---|---|---|
| `locked_by` 已不是本 lockToken，或 `status` 已不是 `processing` | 任务确已被接手或已终结。**所有权已证实易主** | `terminal no-op`。重试归新持有者 |
| 仍属本 worker，但 candidate 已非 `pending` | 已被晋升事务取消或被另一执行推进。**证实本 candidate 已终结** | `terminal no-op` |
| **仍属本 worker、candidate 仍 `pending`，仅租约已过期** | 租约过期但**无人接手**。与 F-Z 的第二行是同一情形 | **`可重试失败`** |
| 续期抛错（数据库故障、连接中断、超时） | **什么都没证实** | **`可重试失败`** |
| 复读本身失败 | 同样什么都没证实 | **`可重试失败`** |

若把后三行当成 terminal 成功，`markTaskSucceeded()` 会命中（它同样只校验 `id AND status AND locked_by`，见 F-Z）并把任务永久标为 `succeeded`，而 candidate 停在 `pending`、`magic_link_requests.resolved_at` 可能仍为空。结果是一次心跳延迟或数据库抖动就静默丢弃一个已受理请求：用户没有链接、没有超龄告警、没有 dead-letter 告警。若 SMTP 恰好在中止前完成，用户还会收到一封**永远无法验证**的邮件。这与 F-Z 是同一个静默丢失的两个入口——一个走 claim 校验，一个走续期——必须同样分流。

判定时机固定为：先中止并拆除 socket（外部副作用优先收敛），再复读并按上表决定。复读**不得**延长预留、不得改写任何行；它只是分类。不得为了「看起来干净」而把不确定情形降级为成功。

**定时器必须在事务 B 打开之前停止（规范性）**：续期只取 candidate 的行锁、不取 advisory lock，而事务 B 会以 `FOR UPDATE` 锁定 F-AC 的最小集（其中必然含同一 candidate）。若定时器在 B 打开后仍存活，一次续期就会去争抢 B 已持有的行锁；由于两者共用同一连接池，在池容量紧张时这会退化为自死锁——B 需要跑完才放锁，而续期占着一条连接等这把锁。因此 `finally` 清除定时器必须发生在事务 B 之前，而不是「handler 返回前的某处」。
...
续期与晋升路径通过 candidate 的行锁串行（晋升在 §5.3b 第 2 步以 `FOR UPDATE` 锁定该邮箱的 pending 行）：要么晋升先提交并把 candidate 置 `cancelled`，随后续期影响 0 行、worker 得知围栏丢失；要么续期先提交，晋升读到未到期预留并 fail closed。两种串行结果都不产生「已是 admin 却仍激活」的状态。

有了续期，**排序不变量不再依赖任何对 SMTP 时长的估计**：只要 worker 存活，预留就覆盖整段调用；worker 若在调用中途死亡，TCP 连接随进程终止，那封信要么已经在死亡之前被 SMTP 服务器接受（线性化于预留到期之前，因而也在晋升之前），要么根本没有发出。
...
1. **崩溃窗口**：worker 在事务 A 提交后、事务 B 之前崩溃，续期随之停止，预留会在最多 `MAGIC_LINK_DELIVERY_RESERVATION_SECONDS` 内阻塞晋升，之后自动放行。排序不变量仍成立：那次 SMTP（若已发生）线性化于进程死亡之前，因而也在预留到期与晋升提交之前；重试的事务 A 会读到 admin 并取消 candidate，0 第二次发送。
2. **晋升等待的真实上界（F-Y）**：v9 声称最坏等待是 `MAGIC_LINK_DELIVERY_MAX_TOTAL_SECONDS`，**那是错的**。两者默认都是 120 s 时，一次 t≈80 s 的续期会把到期推到 t≈200 s，而中止发生在 t=120 s；若失败路径按 v9 那样刻意保留预留，晋升会一直被拒到 t≈200 s，超出承诺值。上面的 F-Y 清除规则消除了这个超出：确认拆除 socket 后立即清除自己写的预留，因此**单次投递尝试**造成的晋升阻塞 ≤ `MAGIC_LINK_DELIVERY_MAX_TOTAL_SECONDS`。

   但**跨重试不是一个全局上界**，规格不再假装它是：任务重试会重新执行事务 A 并重新预留，因此一次晋升尝试可能在同一个 task 的多次投递尝试上连续收到 409。可陈述的真实边界是：(a) 单次尝试 ≤ 硬期限；(b) 两次尝试之间由 `taskBackoffMs` 隔开至少 60 s，其间没有预留，晋升必定成功；(c) 因此运营者按提示重试时，最迟在下一个退避间隙内成功，而不是需要等待整个 dead-letter 生命周期。§11.11 与错误文案都必须按这个措辞，不得再写「最坏等待 = 硬期限」。
...
`magic_link_tokens_pending_cleanup_idx` 继续服务该查询的排序侧。对选出的每个 candidate，再按固定顺序 `task 行 FOR UPDATE → token 行 FOR UPDATE` 打开独立短事务，锁定后复验 kind、protocol v2、`tokenId` 对应、terminal status 与保留期，然后删除。锁顺序与 handler 完全同向，不因反转而改变。

这样做的性质：**驱动集合恰好等于待办集合**。成功投递的 candidate 是 `active`，一开始就不在集合内；仍被 live/retryable task 引用或未过调查窗口的 candidate 被 `exists` 谓词排除在驱动集合之外，因此不会占用批量名额；每成功回收一个，集合就少一个。锁定后的复验仍然保留（谓词与真相之间可能有竞态窗口），但它此时只是防御，不再是收敛的依赖项。因此不需要持久游标或「已处理」标记。

等价的写法是保留 task 驱动并把谓词收窄为「仍引用 pending candidate」；两个方向都成立，**真正起作用的是谓词而不是方向**。本规格固定 candidate 驱动一种，实现不得两种混用。

其余约束不变：每批上限固定为 200 并使用 `FOR UPDATE SKIP LOCKED`；未到 retention cutoff 的项本轮 no-op、以后仍可选。一次失败只记录不含敏感 payload 的 `{ taskId? }` 与错误分类，不改变已提交 task 状态、不重新发送、不阻塞其它项，下一 dispatcher tick 必须再次可选。不能以进程内 “already attempted” 集合永久跳过失败项；删除成功或 candidate 已非 pending 才是该项的幂等收敛。指定 taskIds 的 hook 调用路径不变，仍直接按 ID 处理。
...
本表按 §4.3 的优先级规则是**索引**：只给「情形 → 结果词汇 → 权威条款」。任何细节都在权威处，本表与权威处不一致时以权威处为准。

| 情形 | 结果 | 权威处 |
|---|---|---|
| keyring / SMTP 未配置 | 与基线相同抛 500（目标无关） | §5.2 |
| 公开事务失败 | `整体回滚`（无 intake 行、无任务） | §5.2 |
| intake 任务事务失败 | `可重试失败` | §5.3 |
| 连续 4 次瞬时失败后的第 5 次 | 仍可正常 mint（超龄只判首次 claim） | §5.4 |
| intake 任务耗尽重试 | `永久失败` + `告警` | §5.4 / §5.10 |
| intake 首次执行时已超龄 | `成功 no-op` + `告警`（标记 resolved、不 mint） | §5.4 |
| worker 崩溃（提交前） | `整体回滚` + `可重试失败` | §5.3 |
| worker 崩溃（提交后、标记成功前） | `成功 no-op`（`resolved_at` 非空幂等出口） | §5.3 |
| 陈旧 claim：他人已接手 | `成功 no-op` | **F-Z** |
| 陈旧 claim：租约过期且无人接手 | `可重试失败` | **F-Z** |
| intake 引用的 request 行缺失 | `成功 no-op` + `告警`（durable-state 不变量破坏） | §5.3 / §5.7 |
| 未升级实例领到 intake 任务 | `永久失败`（首次尝试即 dead）——发布门要避免的场景 | §9.2 |
| request-row 清理事务失败 | `只记日志` | §5.7 |
| `recordEvent()` 失败 | `只记日志` | ADR 0002 / #175 |
| SMTP 瞬时失败 | `可重试失败` + `旧链接保持原状`；candidate 保持 pending | §5.3a |
| SMTP 永久失败 / final attempt 耗尽 | `永久失败` + `旧链接保持原状`；终态提交后才回收 candidate | §5.5a |
| SMTP 成功、激活前崩溃 | `可重试失败` + `旧链接保持原状`；允许重发同一 candidate | §5.3a |
| SMTP 成功后 claim stale | `不激活`（按 F-Z 分流终态） | §5.3a / **F-Z** |
| 激活事务失败 | `整体回滚` + `旧链接保持原状` + `可重试失败` | §5.3a |
| 激活提交后、任务成功前崩溃 | `成功 no-op`（active-delivered 幂等恢复） | §5.3a |
| 用户在 SMTP 期间晋升 admin | 由预留互斥，晋升不可提交；崩溃致预留到期时 `不激活` | **F-Q** / **F-X** |
| 晋升事务命中未到期投递预留 | `fail closed 回滚` | **F-Q** |
| 预留续期影响 0 行，复读证实易主 / candidate 已终结 | `中止发送` + `terminal no-op` | **F-X** / **F-AE** |
| 预留续期影响 0 行，但仅本 worker 租约过期、无人接手 | `中止发送` + `可重试失败` | **F-AE** |
| 预留续期抛错，或失败后复读本身失败 | `中止发送` + `可重试失败` | **F-X** / **F-AE** |
| SMTP 超出墙钟硬期限 | `中止发送` + `不激活` + `可重试失败` | §5.3b / **F-W** |
| socket 确认拆除且围栏在己方手中 | 按精确值清除自己写的预留 | **F-Y** |
| 投递期间旧 token 被消费 | `不激活`（candidate `cancelled`）+ `成功 no-op` | **F-P** / **F-AC** |
| 事务 B 复检通过后租约被抢占 | `不可能` | **F-O** |
| SMTP 调用时长超出任何静态估计 | 由预留续期覆盖，不依赖时长估计 | **F-S** |
| 续期与晋升事务并发 | `串行化`（candidate 行锁） | **F-Q** / **F-Y** |
| 受理时 admin、worker mint 前降级 | 按 mint 边界当前角色正常处理 | §2.3 / §5.3a |
| 任一边界读到 admin 后再降级 | 终态不可逆，不复活、不重发 | §5.3a |
| post-finalization cleanup 失败或 hook 前崩溃 | `只记日志`；周期 reconciler 最终重试 | §5.5a |
...
`delivery_reservation_until`（§5.3b）不在表中单列，因为它与 `magic_link_tokens` 行同生共死、且只在 `pending` 期间有值：事务 A 在「+1 `pending`」那一行同时写入它，激活或取消时清空；SMTP 失败时按 F-Y 条件处理——socket 已确认拆除且围栏仍在己方手中就清除自己写的那个值，否则才保持到自然到期。它对请求者同样不可观察，且只在已经 mint 的分支存在——admin 分支根本没有 token 行，因此它不引入任何新的角色相关副作用。

---

## 7. 竞态与并发语义（索引，非规范来源）

同 §5.11：本表是索引，只给「场景 → 结果词汇 → 权威条款」。

| 场景 | 结果 | 权威处 |
|---|---|---|
| 公开路径并发（同邮箱） | 互不阻塞；无共享锁、无唯一约束冲突 | §5.2 |
| 两个 worker 处理同邮箱的两个 intake | `串行化`；恰好 1 pending token + 1 投递任务 | §5.3 |
| 激活事务与 `consumeMagicLinkToken()` 并发 | `串行化`；两种顺序都恰好 ≤1 session | **F-P** / **F-AC** |
| 晋升事务与在途 SMTP 并发 | `fail closed 回滚`；晋升只能线性化在事务 A 前或事务 B 后 | **F-Q** / **F-X** |
| 晋升事务与 pending candidate（无在途投递） | 同事务取消该邮箱 pending candidate 后才写 `role`/`email` | **F-Q** |
| 两个并发晋升事务（改管理员邮箱） | 按规范化邮箱升序取双锁，无反向等待 | **F-Q** |
| 晋升目标邮箱没有 `users` 行 | 角色行锁为空操作；围栏完全由 advisory lock + 预留承担 | **F-V** |
| 事务 B 与租约抢占 / final-attempt sweep 并发 | `不可能`（`SKIP LOCKED` 跳过 / 行锁等待） | **F-O** |
| 清理与未解析 intake 行并发 | `串行化`；未解析行不可删 | **F-N** / §5.7 |
| worker 与 `deliverMagicLinkEmailTask()` 并发 | `串行化`（同一 email advisory lock） | §5.3 |
| 用户在 worker mint 前被提升为 admin | `成功 no-op`；0 token / 0 投递 / 0 SMTP | §5.3 |
| 公开受理时 admin、worker mint 前降级 | 按 mint 边界当前角色正常处理 | §2.3 |
| 任一边界读到 admin 后再降级 | 终态不可逆，必须新请求 | §5.3a |
| worker 崩溃 / 租约过期后重试 | `成功 no-op`（`resolved_at` 幂等出口） | §5.3 |
| 同一 intake 被两个 worker 同时 claim | `串行化`；落败方按 F-Z 分流 | **F-Z** |
| mint 预算并发计数 | 精确，无超发、无多计 | §5.6 |
| 两个清理事务并发 | 互不阻塞、无死锁（`SKIP LOCKED` + 确定性排序） | §5.7 |
| 清理与 pending / processing / retryable handler 并发 | 未解析行不满足清理谓词 | **F-N** / §5.7 |
| 清理与预算窗口交互 | 删除谓词覆盖 `minted_at` 滞后 | §5.7 |
| SMTP 后旧 claim 过期、重复 claim | `不激活`；新 claim 重发同 candidate | §5.3a / **F-Z** |
| 激活后任务完成前崩溃 | `成功 no-op`（幂等恢复） | §5.3a |
| 旧 candidate SMTP 阻塞、较新 candidate 先激活 | 旧 candidate 自身 `superseded`，不覆盖较新 active | §5.3a |
| 多个 eligible replacement（异常/人工恢复） | 仅 `(created_at,id)` 最大者可激活 | §5.3a |
| pending token cleanup 与投递 / 人工 retry 并发 | `串行化`（task → token 行锁）；live/retryable 不可选 | §5.5a |
| task 终态提交与 cleanup hook / reconciler 并发 | 终态先提交再触发 hook；`SKIP LOCKED` 收敛 | §5.5a |
| reconciler 与大量成功投递 / 活候选并存 | 驱动集合已过滤为可处理集合，不饥饿 | **F-T** / **F-AB** |
| 多实例 | 公开路径无共享内存状态；mint 预算跨实例精确 | §5.6 |
| 备份恢复（S7 / `restore/neutralize.ts:290-310`） | 正确性安全（不重复签发、不泄露角色）；见下一行的告警副作用 | §5.3 / §5.4 |
| 恢复后的超龄告警突发 | 预期的确定性假阳性突发，可整体忽略 | **F-J** |
...
4. **中和所有非 active 的 v2 token（新增回滚门，F-AD，必须在滚回旧镜像之前完成）**：旧镜像的 `verifyMagicLinkToken()` / `consumeMagicLinkToken()` **不认识 `delivery_state`**——它们只按「未消费 ∧ 未过期」取行。而 v2 的 `pending` / `superseded` / `cancelled` 转换都**不动 `expires_at`**。因此一旦滚回旧镜像，这些本应永久失效的 token 会**全部复活为可消费链接**，其中最危险的是被 §5.3a F-P 取消的 candidate：那个邮箱刚刚已经用旧链接创建过 session，复活它就正好制造出本设计花了整轮去消除的**第二个 session**；`superseded` 的旧链接同理复活。
   ...
   用「过期」而不是 `DELETE`：保留行便于回滚后审计，且 `expires_at = now()` 正是旧镜像唯一认得的失效方式。该语句必须在第 3 步的 drain 完成之后、第 5 步的镜像滚动之前执行，并把影响行数记入运营记录。执行后必须复查 `delivery_state <> 'active' and consumed_at is null and expires_at > now()` 计数为 0，否则阻断回滚。

   反向说明：`active` 且未消费未过期的行**不得**被这条语句触碰——它们是合法投递出去的链接，旧镜像也应当继续接受，否则回滚会无谓地废掉用户手里正常可用的登录链接。

5. 若要回滚**镜像**：必须在第 2–4 步完成之后再滚动到旧镜像。否则残留的 intake 任务会被旧镜像在**首次尝试**就 `markTaskDead`（机制同 §9.2），对应用户永远收不到链接——v2 §9.3 声称它们「会持续失败直至 dead」、运营者可「直接标记为 dead」，两句都与代码不符：它们**已经**是 dead，且只经历了一次尝试。受影响用户需重新发起请求。
6. 可选：`drop table magic_link_requests;`（保留该表不影响旧代码）。
...
26u. **`续期失败的分类`（F-AE，load-bearing）**：全部断言 socket 在失败瞬间被拆除，然后按复读结果区分终态。`terminal no-op` 两例：(a) 任务被另一 worker 接手（`locked_by` 已变）；(b) 任务仍属本 worker 但 candidate 已被晋升事务置 `cancelled`。`可重试失败` 三例：(c) **仅本 worker 自己的租约过期、无人接手、candidate 仍 pending**——这是最关键的一例，把 `lease_until` 回拨即可构造，断言任务**不是** `succeeded`、`resolved_at` 未被伪造、重试后同一 candidate 仍能正常投递并激活；(d) 续期抛数据库错误；(e) 复读本身失败。若实现直接把「影响 0 行」当成已证实丢失而不复读，(c) 必须 RED——那正是 `renewTaskLease()`（`tasks/index.ts:464-471`）与 `markTaskSucceeded()` 都不校验 `lease_until` 造成的静默丢失。另断言复读不延长预留、不改写任何行。
26r. **`reconciler 不被成功投递饿死`（F-T，load-bearing）**：构造 > 200 个更早的、已 `succeeded` 且 candidate 已 `active` 的 protocol-v2 投递 task，再构造一个**更晚**的 dead task + pending candidate。断言单轮 reconcile 即回收该 pending candidate。若实现改回按 `tasks.updated_at, tasks.id` 扫描 terminal task，该测试必须 RED。另断言 legacy v1 candidate 不被选中。**再加一例（F-AB）**：构造 > 200 个**更旧**且仍活着的 pending candidate（其投递任务分别为 `pending`、`processing`、可重试 `failed`、以及已终态但未过调查窗口），外加一个**更新**的 dead + 已过保留期 candidate；断言单轮 reconcile 仍然回收后者。若驱动查询退回「只按 `delivery_state='pending'` 取最旧 200」，该用例必须 RED。
...
- **`tasks`**：每个被受理的请求另产生一行 `tasks`。v9 为 `succeeded` 的 `auth.magic_link_request` 增加了有界保留清理（§5.7 F-U），因此本 Issue 引入的这条增长是收敛的。`dead` intake task 有意保留待查，其行数必须纳入监控；`auth.magic_link_email` 与其它 kind 的 terminal task 累积是**基线既有行为**，本 Issue 不处理，由实现 PR 登记的通用 `tasks` 保留作业后续 issue 承接。
...
- **在途投递会持续续期，晋升的最坏等待由硬期限决定**：由于预留随 SMTP 调用续期（§5.3b F-S），一次缓慢或被恶意 trickle 拖住的投递会把晋升阻塞到 `MAGIC_LINK_DELIVERY_MAX_TOTAL_SECONDS`（默认 120 s）为止——**这是单次投递尝试的界，不是全局界**。任务重试会重新预留，所以同一个 task 的多次尝试可能连续拒绝晋升；但两次尝试之间至少隔 `taskBackoffMs` 的 60 s 且期间无预留，晋升在该间隙必定成功（§5.3b 残余 2）。运营提示与错误文案必须按这个措辞写，不得声称「最坏等待 = 硬期限」。该值同时决定 dispatcher 的最坏队头阻塞时长，选值理由见 §8.2a。这是排序正确性的直接代价：要么允许晋升插到在途发送中间（v8 的错误），要么让运营操作等待一个有界窗口。调大硬期限会同步加长该等待，调小则会更早把慢投递判为结果不确定（§5.3b 残余 2）。
...
| `src/db/migrations/0031_magic_link_requests.sql` | 新增（§5.5/§5.5a DDL、`delivery_reservation_until` 列与 CHECK、既有 token backfill、queue class 约束、reconciler 用的 `tasks_magic_link_delivery_token_idx` 部分表达式索引） |
...
| `src/lib/env.ts` | 新增七个变量与镜像常量 `SMTP_HANDSHAKE_IDLE_BUDGET_MS`；在 `assertRuntimeSecurity()` 内实现保留期、intake 门/cap 组合、预留下界、硬期限四项 fail-closed 校验与 dedupe/TTL 告警；导出仅测试使用的 `__resetEnvCacheForTests()` |
...
| `src/modules/auth/magic-link.integration.test.ts` | 切片 1–4a 的真实 PostgreSQL 测试，含 admin promotion/demotion、晋升围栏（26o）、预留续期与硬期限（26q）、reconciler 饥饿（26r）、事务 B task 行锁（26p）、投递期消费取消（26i）、intake 行锁与 intake task 保留（19a/19b）、旧 SMTP/新 active 围栏及 cleanup 引用竞态 |
...
| `.env.example` | 新增七个变量及注释，明确开关仅精确字符串 `true` 生效；说明预留窗口由续期覆盖整段调用、硬期限决定晋升最坏等待，且**不得**把三个 SMTP 超时之和理解为调用上界 |
...
- 复核清单必须逐条确认。按 §4.3 的优先级，本清单也是**索引**：每项只给「要确认的可观测事实 + 权威条款」，不复述条款内容。带 F-编号的项，判定标准以该条款为准。

  **不可区分性（§4.1 G1–G10）**
  1. 公开请求路径不存在任何 `users` / `magic_link_tokens` / `tasks` 查询与按邮箱锁（§5.2）；
  2. 事务内往返恒为 4 次、事务外恒为 2 次，且均与目标无关（§5.2）；
  3. 任何路径都不会因角色返回非 200（§5.2 / G1）；
  4. 不存在目标相关的、请求者可观察的限流状态（G3）；
  5. `request-code-email-ip:*` 已在 Magic Link 全路径解除引用（G4）；
  6. `magic_link_requests` 列集合与 §5.5 完全一致（无原因列、无角色列）；
  7. 事件 / 日志 payload 不超出 §5.10 白名单，且不含角色或抑制原因；
  8. 代码、注释、PR、CHANGELOG 中从未把 `auth.magic_link_request` 称为 delivery task（§2.2）。

  **admin 边界与晋升围栏**
  9. mint 边界读到 admin 时 token 与投递任务计数恒为 0；SMTP 紧前读到 admin 时 0 发送（G5）；
  10. 排序不变量成立：晋升提交后不再为该邮箱发起 SMTP，且在途发送不会被激活（**F-Q**）；
  11. `setupSite()` 与 `changeAdminEmail()` 是仅有的两条 admin 晋升写路径，二者都实施围栏；其它 `update(users)` 均不写 `role`/`email`（**F-Q**）；
  12. 实现与文档均未把 unknown 邮箱的角色行锁当作串行化手段（**F-V**）；
  13. 围栏丢失时立即中止发送，未拖到硬期限（**F-X**）；
  14. 预留清除带任务 claim 与精确值双重保护（**F-Y**）；
  15. 续期失败按原因分流终态（**F-AE**）。

  **锁与围栏**
  16. intake 行与 task 行都以 `FOR UPDATE` 锁定并持有到提交（**F-N** / **F-O**）；
  17. §5.3 step 1 的 claim 校验按 id 锁定后在应用层判定，谓词未写进 `WHERE`（**F-Z**）；
  18. 陈旧 claim 按 F-Z 分流，未一律收敛为成功（**F-Z**）；
  19. 激活事务的 token 锁集合是有界最小集，且 `consumed_at IS NULL` 谓词存在（**F-AC**）；
  20. 锁顺序全局同向 `task → magic_link_requests → advisory(email) → token → user`，reconciler 的 `task → token` 与之同向（§5.3）；
  21. 清理不在主事务内，且只删 `resolved_at is not null` 的行（§5.7）。

  **delivery lifecycle**
  22. replacement mint 只创建 pending token，绝不修改旧 active token；verify/consume 固定 active + delivered 谓词（§5.3a）；
  23. SMTP 在事务外；激活事务重检 claim、candidate、role、预留、投递期消费与单调围栏（§5.3a）；
  24. 失败、回滚、崩溃、stale lease 均保留旧 active token；激活后崩溃幂等完成（§5.11）；
  25. delivery payload 以显式 `deliveryProtocol: 2` 分流；legacy task 始终走基线 SMTP（§5.3a）；
  26. 文档与实现均未宣称 SMTP exactly-once，也未宣称旧 token 过期后仍保证可用（G10 / §8.1）。

  **清理、迁移与发布**
  27. reconciler 驱动集合已过滤为终态且过保留期的引用（**F-T** / **F-AB**）；
  28. `succeeded` intake task 有界清理，`dead` 保留待查（**F-U**）；
  29. `markTaskDead`、failed-to-dead、final sweep 均在终态提交后触发 hook；周期 reconciler 覆盖失败与崩溃（§5.5a）；
  30. migration / backfill 保持历史与旧代码新插入 token 可用且不延长 expiry（§5.5a）；
  31. `db-reset.ts` 已包含新表（§9.1）；
  32. 回滚在滚回旧镜像前已过期全部非 active v2 token，复查计数为 0，且未误伤 `active` 未消费行（**F-AD**）；
  33. 回滚 drain 按 §9.3 执行，dead intake 逐项留证处置；
  34. env 校验四项 fail-closed 与 dedupe 告警均在 `assertRuntimeSecurity()` 内；mint 上限回落只在 `rate-limit-policy.ts`（**F-E** / **F-M** / **F-R**）；
  35. 硬期限的队头阻塞代价已被知情接受（**F-W**）；
  36. `magicLinkDeliveryInFlight` 三个目录齐备（**F-AA**）；
  37. 切片 4a 的真实 PostgreSQL 并发、回滚、stale lease、consume race、promotion/demotion、migration 与 cleanup 测试均实际通过（§10）。
