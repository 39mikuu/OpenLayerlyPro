...
- **状态**：Proposed（v26，按三轮独立复核 F1–F12、F184-01–F184-15、F-A–F-AM 及 Codex 多轮复核修订；规则以 R-编号索引，见 §5 引用约定。v26 将 cleanup/list/count/verify-zero 全部门禁暴露为 production bundle 子命令）
...
- 复核清单必须逐条确认：
  ...
  24. 回滚 drain 的 cleanup-aged-null/list-terminal/count/verify-zero/abandon 全部由 production bundle 暴露并按固定命令链执行；最终从空 cursor 复列且 terminal-pending verify-zero 成功；非空 reservation 仍只在 F-Y/quiescence 证据后取消。
