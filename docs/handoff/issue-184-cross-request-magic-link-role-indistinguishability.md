...
- **状态**：Proposed（v25，按三轮独立复核 F1–F12、F184-01–F184-15、F-A–F-AM 及 Codex 多轮复核修订；规则以 R-编号索引，见 §5 引用约定。v25 令 F-AG 无条件分页枚举全部 terminal v2 pending candidate，并要求 cleanup/处置后的零计数复查）
...
| `src/modules/auth/magic-link-rollback.ts` + integration test | F-AG 全 terminal-v2-pending keyset list（无 cutoff，201/200）、处置后从头复列与 count=0 门禁；两模式与审计 |
...
- 复核清单必须逐条确认：
  ...
  24. 回滚 drain 先排空可重试任务并循环 cleanup aged-NULL 至 batch/count=0；F-AG 随后枚举全部残余 terminal v2 pending（不看 age/reservation），处置后从头复列并要求总数 0；非空项仍只在 F-Y/quiescence 证据后取消。
