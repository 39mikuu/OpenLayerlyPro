...
**可执行架构(明确)**——backfill **必须复用与上传完全相同的规范化管线**(同一 §2 `normalizeRasterImage`、storage、db、tasks、env),**严禁**在脚本里重写一份(会与上传 drift):
- 把 §2 的规范化核心抽成**纯模块** `src/modules/file/normalize.ts`(只依赖 sharp + 常量/env,不引 Next server-only),供上传入口与 backfill 共用。
...
- [ ] 规范化核心抽成共享 `normalize.ts`,上传与 backfill **同一管线**(无 drift)
