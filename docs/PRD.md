...
当前发布状态：v1.0 pre-release final acceptance。S1a/S1b/S2/S3/S4/S5/S6/S7 与订阅已实现；#88 已关闭，剩余主线是 #104 对审计修复后冻结候选（含 #119）的 exact-main 重验收。▶
...
已完成：MVP → 安全基础 → 配置中心 → 网络边缘 → Integration 基座 → Theme v1 基座 → UI i18n → Content i18n/AI → v1 Core hardening → 自动支付/订阅/文件能力 → S1a–S7。

当前：v1.0 最终验收与发布 #104（原 #88 已关闭，矩阵按审计后候选重跑，含 #119）。

后续：Plugin v0 → Hub official plugin → 多实例负载均衡与高可用。
