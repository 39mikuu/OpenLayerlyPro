...
> 状态标注约定：✅ 已实现（当前代码可用）｜▶ 当前发布主线｜🚧 后续计划。本文档描述当前 `main`；历史实现过程见 ADR/handoff，当前版本计划与发布门槛见 `release-v1.2-plan.md`。
...
当前发布状态：v1.1.0 已于 2026-07-17 发布（tag `3a80b34`）。v1.2 的 M1–M4 实现与 M5 验收收尾/发布准备均已进入 `main`，但尚未创建 v1.2 tag 或 GitHub Release；剩余正式发布门槛见 `docs/release-v1.2-plan.md`。▶
...
- **会员**：等级、按笔时间窗、active/suspended/revoked 生命周期、并发串行授予、按付款期反转，以及按当前 tier 实时解析的 Core 白名单权益。✅
...
- **通知**：新内容邮件通知默认关闭、粉丝显式 opt-in、一键退订、按收件人 locale 渲染、campaign/delivery 账本、通知预算/pacing、同步 SMTP permanent rejection 抑制，以及后台安全聚合观测。✅
- **赞助者鸣谢墙（WP5）**：默认关闭、粉丝显式 opt-in 的公开支持者页；资格按 effective active membership 每次请求派生（不缓存、无定时下墙任务），重叠会员取最高等级；展示显示名与等级、不展示金额/邮箱；一次性纯文本献词先审后展示、URL 不链接化；创作者可关闭、按等级限制、批准/隐藏；每用户一条 entry 由数据库唯一约束保证，状态转换与审核动作全部入审计。✅
- **Session / Auth**：HMAC 会话、管理员账号维护、粉丝高熵验证码、邮件 Magic Link、Google / GitHub OAuth、Turnstile、可信 IP 与 S4 限流。✅
- **配置中心**：`app_settings` 加密配置、revision/CAS 并发保护，以及后台 SMTP/Turnstile/S3/上传/Stripe/Google OAuth/GitHub OAuth/Translation 管理；按各组契约支持 DB、环境变量与默认值。✅
...
- WP5 起主题契约含必选 `SupporterWall` 组件槽，三内置主题同步实现；主题仍只消费 Core 派生的 view-model，不旁路取数。✅
...
| 新内容邮件通知 opt-in、退订、通知预算/pacing、同步 SMTP permanent rejection 抑制；投递为 at-least-once，不承诺不重复投递 | ✅ |
...
当前：v1.1.0 已发布；v1.2 的邮件 Magic Link、Google/GitHub OAuth、Membership Bundle、G5/G7 债务包和验收收尾/发布准备均已合并。正式发布仍须在 exact release HEAD 完成独立门槛并创建 tag / GitHub Release；详见 `docs/release-v1.2-plan.md`。

后续：v1.2 发布后根据真实使用确定 v1.3 范围。G3 legacy compatibility removal 不早于 2026-10-14；多实例负载均衡与高可用继续按路线图条件推进。
