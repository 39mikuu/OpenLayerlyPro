...
当前发布状态：v1.1.0 release prep in progress at `97547b6`。M1-M4 已合并，package version 已冻结为 1.1.0；M5 验收仍待真实 SMTP、部署 dogfood、`v1.0.0` 原地升级和带通知 key 的 backup/restore drill 完成。▶
...
当前：v1.1「不只画师」继续提高产品完成度；WP1 已完成，WP2 新内容邮件通知已交付代码与文档同步，真实 SMTP / 部署 dogfood 仍列为发布门槛；WP5 赞助者鸣谢墙已交付代码与文档同步；后续 WP 按计划书里程碑串行推进，不再等待固定发布后窗口。

后续：v1.1 WP6 → Phase 8 运营完成度与官方内置能力 → 多实例负载均衡与高可用。（Hub / 聚合发现暂不规划。）
