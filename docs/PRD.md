...
当前发布状态：v1.0 pre-release final acceptance。S1a–S7、订阅、审计修复、#58 与 #119 已实现；#64 已作为实现 epic 关闭。操作者已完成真实 Stripe、SMTP、S3/R2、Turnstile/CSP、密钥托管与恢复验收。当前唯一主线 #104 负责证据归档、最终候选 SHA 冻结、exact-final-SHA CI/受影响范围复验和发布授权。▶
...
- **配置中心**：`site_settings` 保存普通配置与加密配置封装，后台管理 SMTP/Turnstile/S3/上传/Stripe/Translation；按各组契约支持 DB、环境变量与默认值。✅
- **审计与任务**：统一 `audit_events` 因果链、`app_events` 运维事件、durable task/outbox、lease/fencing、有界重试与后台重试视图。✅

Core 边界原则：

1. 所有可选集成关闭时，人工付款与本地 Core 仍能运行。
2. 权限、付款、会员和文件生命周期规则只存在于 Core，不下沉到 Theme 或未来 Plugin。
...
- 第二主题、后台活动主题选择器、主题切换 audit 与双主题视觉回归门禁属于 v1.1 WP1。Draft PR #123 尚未满足合并要求，不能按当前状态视为已实现。🚧
...
- Plugin 是后期第三方扩展机制，允许社区在不修改 Core 的前提下扩展功能。
- 必须有明确能力声明、权限隔离、故障隔离与可禁用边界。
...
| 需求 | 状态 |
|---|---|
| 生产环境禁止默认 / 弱 `SESSION_SECRET`；Compose 可持久化自动生成并支持 env/file override | ✅ #119/#120 |
...
已完成：MVP → 安全基础 → 配置中心 → 网络边缘 → Integration 基座 → Theme v1 基座 → UI i18n → Content i18n/AI → v1 Core hardening → 自动支付/订阅/文件能力 → S1a–S7 → 审计修复与 #119。

当前：#104 归档已完成的外部验收，冻结最终候选，完成 exact-final-SHA CI/受影响范围复验并发布 v1.0.0。

后续：v1.1「不只画师」稳定窗口后启动 → Plugin v0 → Hub official plugin → 多实例负载均衡与高可用。
...
5. 上述外部验收已由操作者完成；#104 仍须补齐可审计的日期、环境、证据链接与测试 SHA，并绑定最终候选。
6. 完整 CI、浏览器 E2E、恢复 E2E 和安全告警检查全绿后才创建 `v1.0.0`。
7. 权威清单见 [release-v1.0-checklist.md](release-v1.0-checklist.md)。