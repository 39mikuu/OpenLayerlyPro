...
- **通知**：新内容邮件通知默认关闭、粉丝显式 opt-in、一键退订、按收件人 locale 渲染、campaign/delivery 账本、通知预算/pacing、同步 SMTP permanent rejection 抑制，以及后台安全聚合观测。✅（真实 SMTP / 部署 dogfood 仍属 v1.1 发布门槛）
...
| 新内容邮件通知 opt-in、退订、通知预算/pacing、同步 SMTP permanent rejection 抑制；投递为 at-least-once，不承诺不重复投递 | ✅（真实 SMTP / dogfood 仍属 v1.1 发布门槛） |
...
当前：v1.1.0 已发布；v1.2「登录与会员权益完成度」正在规划并串行推进：M1 邮件 Magic Link、M2 Google / GitHub OAuth、M3 Membership Bundle、M4 G5/G7 债务包、M5 验收与发布。v1.2 不设置固定等待窗口，不搭载 G3 legacy compatibility removal；dispatcher query optimization 已属 v1.1.0 baseline，v1.2 只保持回归绿。

后续：v1.2 验收后按真实使用确定 v1.3 主题；G3 legacy compatibility removal 最早进入 2026-10-14 之后的首个 release（预期 v1.3）；Phase 8 官方内置能力继续按明确 Core/Integration 需求推进，再到多实例负载均衡与高可用。（Hub / 聚合发现暂不规划。）
