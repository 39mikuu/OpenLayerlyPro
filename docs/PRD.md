...
>
> 本文档是**产品定位与范围取舍**层面的说明。逐条可验证的完整需求基线（功能需求、数据不变式、接口契约、非功能需求与验收标准）见
> [软件需求规格说明书（SRS）](./requirements-spec.md)。
...
当前发布状态：v1.1.0 已于 2026-07-17 发布（tag `3a80b34`）；v1.2 正在规划，范围与门槛见 docs/release-v1.2-plan.md。▶
...
- v1.2 计划增加邮件 Magic Link、Google OAuth 与 GitHub OAuth，作为粉丝登录补充入口；现有邮箱验证码登录保留为 fallback，管理员登录不切换到第三方 OAuth。
- v1.2 计划在 `membership_tiers` 上增加白名单化的 Membership Bundle；Tips、PPV 等支付型新商业能力继续保留为产品定义和运营验证项。
...
当前：v1.1.0 已发布。v1.2 依次推进邮件 Magic Link、Google/GitHub OAuth、Membership Bundle、G5/G7 债务包和发布验收；详见 `docs/release-v1.2-plan.md`。
