...
1. **首个参考适配器 = Stripe**。支付宝/微信需 ICP 备案域名 + 商户资质,开通/测试摩擦大,留作 provider 抽象下的后续扩展。Stripe 测试态(test keys)可做真集成测试。
2. **v0.2 只做一次性付款**;自动续费/订阅推迟到后续里程碑。
3. provider 抽象首版只需 `stripe` 一个适配器跑通,但接口与注册表要为 `alipay`/`wechat` 预留,不写死 Stripe-only。
