...
> Theme v1 基座已落地：主题数据契约、全部公开页面契约化、内置主题标准化，以及明暗切换、字体、颜色预设与自由取色。第二主题、安装式主题包和主题切换 UI 属于 v1.0 之后的按需扩展，不是当前 S6 → S7 → #88 发布主线。
...
第二主题、主题包生命周期、后台主题选择器和第三方主题安装边界均推迟到 Plugin/Theme 后续阶段。只有存在第二个受支持主题且能力边界重新评审后，才应启用活动主题切换 UI。当前发布状态与唯一主线见 [../roadmap.md](../roadmap.md)。
