...
- `src/modules/theme/`：主题数据契约（`SiteChromeView` / `HomeView` / `PostListView` / `PostDetailView`）、`Theme` 描述符（组件槽）、注册表与 `getActiveTheme()`——读 `site_settings.theme`，未知/缺省回落内置主题（Core 配置驱动切换的接缝；第三步后注册表含 `builtin`/`blog` 两主题，见下）。
