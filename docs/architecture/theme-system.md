...
> Theme v1 基座已落地：主题数据契约、全部公开页面契约化、内置主题标准化，以及明暗切换、字体、颜色预设与自由取色。第二主题（Blog）与后台活动主题选择器已在 v1.1 WP1 落地（见「第三步」）。安装式主题包与主题市场暂不规划；后续主题扩展优先作为官方内置主题能力评估。
...
- `src/modules/theme/`：主题数据契约（`SiteChromeView` / `HomeView` / `PostListView` / `PostDetailView`）、`Theme` 描述符（组件槽）、注册表与 `getActiveTheme()`——读 `site_settings.theme`，未知/缺省回落内置主题（Core 配置驱动切换的接缝；第三步后注册表含 `builtin`/`blog`/`wordpress` 三主题，见下）。
...
- **主题颜色预设（站点级）**：`Theme.colorPresets` 保存具名预设的 hue；活动预设存 `site_settings.theme_config`。内置主题未配置时默认 `blue`；`neutral` 只是可选的零覆盖预设（hue 为 null，不注入覆盖）。Blog 主题未配置时默认 `ink`，同样为零覆盖。
...
- **后台**：`/admin/site`「外观」卡 + `GET/PUT /api/admin/theme`；PUT 只接受活动主题的预设 id，或 `custom` + `[0, 360)` 整数 hue——**禁止提交任意 CSS 变量/值**（第三步后 PUT 另接受可选 `theme` 字段用于切换活动主题，见下）。
...
- **WordPress 经典主题（`src/themes/wordpress/`）**：第三个内置主题，作为 WP1 follow-up 单独授权提前实现。采用传统 WordPress 博客形态：桌面主栏 + 右侧栏、文章摘要流、经典单篇阅读页；交易/账号页面继续复用内置主题槽。配色使用同一主题内的两个精确变量预设（`gofun-seiji` 与 `layer-seal`），不开放任意 CSS 变量输入，也不新增运行时主题加载面。

- **注册表**：`ThemeId = "builtin" | "blog" | "wordpress"` 编译期联合类型；主题为静态注册表条目（一等公民代码，无运行时加载/上传面），`resolveThemeId` 未知回落不变。
- **配置分键**：`site_settings.theme_config` 改为按主题 id 分键（`{ builtin: {...}, blog: {...}, wordpress: {...} }`），各主题的颜色选择独立保存、来回切换不丢失；旧平铺形态读取时归属内置主题、首次写入时自动迁移，无数据库迁移。
...
- **视觉回归基线（known-gaps G4/G6）**：`src/modules/i18n/key-completeness.test.ts` 显式校验 zh/en/ja key 集合一致；`e2e/theme-visual-baseline.spec.ts` 覆盖 Home/Posts/PostDetail × 明暗 × `builtin`/`blog`/`wordpress` 三主题，并额外覆盖 WordPress `layer-seal` 预设首页和 390px 移动端三页。

## Deferred follow-up

主题包上传、第三方主题安装与主题市场暂不规划。未来如真实运营证明需要扩展主题分发，应作为官方内置主题能力或独立产品方向重新评估，不依赖通用第三方 Plugin runtime。当前发布状态与唯一主线见 [../roadmap.md](../roadmap.md)。
