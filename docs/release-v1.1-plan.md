...
> 状态：**已完成并归档**。v1.1.0 已于 2026-07-17 从 `3a80b34` 发布；M5 验收证据见 [v1.1.0 release notes](./releases/v1.1.0-release-notes.md)。本文档保留 v1.1 的范围、实施顺序和验收标准，不再作为当前待办清单。
...
```text
M0  [已完成] v1.0.0 发布 + 发布后审计硬化
M1  [已完成] G4 i18n 完整性门禁 + G6 视觉基线 + WP1
M2  [已完成] WP3 RSS + WP4 SEO
M3  [已完成] WP2 邮件通知
M4  [已完成] WP5 鸣谢墙 + WP6 自托管访客统计
M5  [已完成] v1.1 验收与发布
```

M1-M5 均已完成；v1.1.0 的验收与发布证据已归档。
...
- [x] 全部纳入范围的 WP 验收通过；CI 与 restore drills 证据已归档。
- [x] G4 i18n key 完整性和 G6 主题视觉回归通过。
- [x] feed、sitemap、通知路径及 WP5 鸣谢墙完成跨切面权限测试和有界性检查。
- [x] 邮件通知完成真实 SMTP、失败处理、预算、退订和事务优先级验证。
- [x] WP2 campaign 事务围栏、唯一约束、优先级/保留容量和并发恢复通过真实 PostgreSQL 测试。
- [x] zh/en/ja UI、邮件和公开页完成同步。
- [x] 从 v1.0.0 原地升级与备份恢复演练通过。
- [x] CHANGELOG、PRD、roadmap、admin/deployment 文档和三语官网完成发布同步。
