...
> 状态：**WP1 实现与验收完成并已合并（PR #123，exact-head CI run #554 通过），WP2–WP6 提案未启动**。本文档定义 v1.1 的目标、产品优先级、实施顺序与发布门槛。
...
- **实现要点**
  ...
  - SMTP 投递语义为 at-least-once：唯一约束不等于 exactly-once；SMTP accepted 后、任务标记成功前进程崩溃仍可能造成邮箱重复，产品与运维文档不得承诺“不重复投递”。
...
- **验收**
  - [ ] 默认不泄露身份；未设置显示名不展示。
  - [ ] user id 唯一约束和并发测试证明每个用户最多一条 entry；待审核、已批准、已隐藏状态及转换有测试，未批准内容在页面和任何页面缓存中均不可见。
  - [ ] 输入纯文本安全、URL 不可点击、无 HTML 注入。
  - [ ] effective membership 重叠取最高 level；tier 停售不下墙；到期、暂停、撤销和付款反转后下一次公开查询立即下墙。
  - [ ] 显示名写接口通过 auth-before-body 与有界 body 门禁，且无邮箱 fallback。
  - [ ] `builtin`/`blog`/`wordpress` 三主题及 zh/en/ja 通过 G4 key 完整性；G6 增加代表性桌面/移动端鸣谢墙基线，以及三主题下共享 Core 页面的渲染/权限 smoke。只有实际新增 Theme component slot 时才要求三主题组件身份断言。
- **预估**：中。
...
M2  WP3 RSS + WP4 SEO
M3  WP2 邮件通知
M4  WP5 鸣谢墙；在进入 M5 前明确决定 WP6 纳入本版或顺延 v1.2
M5  v1.1 验收与发布（仅验收 M4 结束时明确纳入的 WP；WP6 未纳入时不得作为发布阻塞）
```

M1 已完成。M2–M5 目标总体量为 6–8 周，不含稳定窗口。超出预期时先砍 WP6，再将 WP5 顺延 v1.2；WP1–WP4 是不可减内核。
...
- [ ] 全部纳入范围的 WP 验收通过；实际 CI 门禁全部完成：`pnpm lint`、`pnpm format:check`、`pnpm check:request-bodies`、`pnpm check:auth-before-body`、`pnpm exec tsc --noEmit`、`pnpm db:migrate`、`RUN_DB_INTEGRATION_TESTS=true pnpm test`、`pnpm build:migrator`、`pnpm build:files-backfill`、`pnpm build:admin-reset`、`pnpm build:restore-tools`、对应产物的 `node --check`、`scripts/test-backup-app-state.sh`、`scripts/test-restore-shell-args.sh`、`scripts/test-entrypoint-secrets.sh`、restore scripts `shellcheck`、`pnpm build` 与 `pnpm test:e2e`。
- [ ] G4 i18n key 完整性和 G6 已落地主题视觉回归持续为绿，并扩展覆盖 WP5 新公开表面。
...
| SMTP at-least-once 造成重复邮箱 | 文档明确非 exactly-once；delivery/任务创建幂等，accepted 后崩溃故障注入验证恢复行为 |
| 批量邮件损害信誉 | 严格 opt-in、退订、pacing、同步永久失败抑制、SPF/DKIM/DMARC 文档；RSS 分流 |
| 稳定窗口被功能工作侵占 | 除文首已记录的 WP1 例外外，窗口内不启动 WP2–WP6；起点以窗口实际结束为准 |
| 单人维护并行过多 | 按 M2→M5 串行；WP6 预设可砍 |

已解决历史（不再作为 v1.1 未决风险）：

- WP1 已完成主题契约盘点并同步 `builtin`/`blog`/`wordpress`；G4/G6 已进入 CI。后续只要求门禁持续为绿，并为 WP5 新公开表面补充对应覆盖。
