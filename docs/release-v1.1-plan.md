...
> 状态：**WP1 实现与验收完成并已合并（PR #123，exact-head CI run #554 通过），WP2–WP6 提案按本文档里程碑继续推进**。本文档定义 v1.1 的目标、产品优先级、实施顺序与发布门槛。
> 前置条件：`v1.0.0` 正式发布（已于 2026-07-06 完成，tag/Release/#88/#64 均已关闭）。不再设置固定发布后等待窗口；真实使用者不希望长期停留在半成品状态，后续工作按需求明确度、实现风险和验收质量推进。
>
> **决策记录（2026-07-08）**：第三主题 `wordpress`（WordPress 经典）作为 WP1 follow-up 实现；该记录仅覆盖第三内置主题与配套文档/测试。
>
> **决策记录（2026-07-07）**：维护者在发布次日启动 WP1（第二主题 + 后台主题选择器）。WP1 已按 §3 WP1 的全部验收项与合并前置门禁（G4、G6）完成验收并合并。
...
M1 已完成。M2–M5 目标总体量为 6–8 周。超出预期时先砍 WP6，再将 WP5 顺延 v1.2；WP1–WP4 是不可减内核。
