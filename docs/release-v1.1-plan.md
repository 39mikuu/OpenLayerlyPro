...
> 状态：**M1-M4 已合并（`97547b6`）；M5 验收证据已于 2026-07-16 全部收集**（真实 SMTP 含 pacing/预算/operator 故障恢复、部署 dogfood、`v1.0.0` 原地升级演练含强制 file-safety remediation、带通知 key 的 backup/restore drill），证据记录于 [v1.1.0 release notes](./releases/v1.1.0-release-notes.md)；发布 PR 合并后打 tag 完成发布。本文档定义 v1.1 的目标、产品优先级、实施顺序与发布门槛。
...
M1-M4 已完成并合并；M5 验收与发布进行中。WP6 不再按默认溢出项自动顺延；若后续需顺延，须另行记录维护者决策。WP1-WP6 均已进入 v1.1 验收范围。
