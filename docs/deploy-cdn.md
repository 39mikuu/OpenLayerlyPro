...
- `/feed.xml`、`/sitemap.xml`、`/sitemaps/*`、`/robots.txt` 是公开、动态但可 CDN 缓存的输出；CDN 必须遵守应用返回的 `Cache-Control`（短 `s-maxage`），不要为其他动态或私有路由扩大 public 缓存范围；
...
生产环境解析失败时认证入口会失败关闭并告警，不会把不同访客放入共享认证桶；文件读取仍使用各操作独立的高阈值 unresolved emergency bucket。上线前必须从真实 CDN 路径验证可信 IP 解析。
