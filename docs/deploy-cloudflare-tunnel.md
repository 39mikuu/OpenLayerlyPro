...
不要只执行 `git pull && docker compose up`。按[升级指南](deployment/upgrade.md)停止 app、处理 duplicate pending payment、运行 one-off migrator 与 mandatory file-safety backfill 后再启动。当前 `backup.sh` 只依据容器环境变量 fallback 判断 active storage，不能识别后台 DB override；备份也无法把 DB override / 混合 local+S3 的历史文件收进单一完整对象归档——两侧存储都要自行保护，详见[备份与恢复](deployment/backup-restore.md)。
