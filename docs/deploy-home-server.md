...
- 生产环境无法解析可信客户端 IP 时，`admin-login`、`request-code`、`verify-code`、Magic Link 和 OAuth 会以 503/通用登录错误失败关闭，不再把不同访客压进共享认证桶。开发/测试仍可使用各操作专用的 unresolved emergency 桶；文件下载保留独立高阈值降级桶。上线前必须按真实代理拓扑配置并验证 `TRUSTED_PROXY_HEADER` / `TRUSTED_PROXY_HOPS`。
