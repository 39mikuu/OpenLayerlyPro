...
- 登录码使用 6 位数字 + 浏览器 256-bit challenge、每 code 5 次 challenge-bound 尝试上限、keyed email identity、错误后记账和 source-scoped pre-comparison budget。详见 [登录码 challenge handoff](handoff/login-code-6-digit-challenge.md) 与 [S4 handoff](handoff/harden-s4-auth-rate-limiting.md)。
...
S7 #87 已把 archive 校验、旧 schema probe、恢复任务中和、DB-aware storage inventory 和 DB↔存储收敛纳入正式恢复流程（见[备份与恢复](deployment/backup-restore.md)）；#88 已在 v1.0.0 exact release HEAD 完成当时的真实环境恢复验收，历史门槛见 [v1.0 清单](release-v1.0-checklist.md)。后续升级仍须按目标版本的当前门槛重新演练，不能复用旧发布结论。
