...
使用 S3 / R2 时脚本会跳过本地 uploads；对象存储 bucket 仍需单独启用版本控制或 provider 备份。升级或恢复前必须阅读[备份与恢复](docs/deployment/backup-restore.md)和[升级指南](docs/deployment/upgrade.md)。v1.0 最终发布还要求 #87 的一致性恢复流程通过验收。
